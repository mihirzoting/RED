console.log('[RED] history-premium.js loaded, constants:', {
  url: typeof SUPABASE_URL !== 'undefined' ? 'set' : 'MISSING',
  anonKey: typeof SUPABASE_ANON_KEY !== 'undefined' ? 'set' : 'MISSING',
});
...
async function fetchPremiumData(filter) {
  ...
  var url = SUPABASE_URL + '/rest/v1/refine_history?user_id=eq.' + user.id + '&order=created_at.desc&limit=200';
  if (startDate) {
...
async function clearHistoryAction() {
  ...
  try {
    var res = await fetch(
      SUPABASE_URL + '/rest/v1/refine_history?user_id=eq.' + user.id,
      {
        method: 'DELETE',
        headers: {
          apikey: SUPABASE_ANON_KEY,
