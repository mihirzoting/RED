...
function injectPanel() {
  ...
  function reapplyTokens() {
    style.textContent = buildStyles(borrowClaudeTokens());
  }
  ...
  pasteBtn.addEventListener('click', () => {
    if (window.__RED.pasteToClaude) window.__RED.pasteToClaude();
  });
...
function updateAnalysis(analysis) {
  ...
  if (tokenLine) {
    const cost = analysis.costEstimate && analysis.costEstimate.length > 0
      ? analysis.costEstimate[0].inputCost
      : '';
    let line = `${tokenCount} tokens`;
    if (cost) line += ` · $${cost}`;
...
function pasteToClaude() {
  const refined = panelRef?._lastRefined;
  if (!refined) return;
...
async function startUpgrade() {
  try {
    ...
    const res = await fetch('https://votjuphsggdecoawqeqc.supabase.co/functions/v1/create-payment-link', {
      method: 'POST',
      headers: {
...
window.__RED.pasteToClaude = pasteToClaude;
...
async function renderHistoryView(containerEl) {
  ...
  if (user && token) {
    try {
      var res = await fetch('https://votjuphsggdecoawqeqc.supabase.co/rest/v1/users?id=eq.' + user.id + '&select=user_type', {
        headers: {
          apikey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZvdGp1cGhzZ2dkZWNvYXdxZXFjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE4NTE3OTQsImV4cCI6MjA5NzQyNzc5NH0.fZW6kk62ka7glAPbJiNrqrh2UU84v3RNuR_e5w04mKE',
          Authorization: 'Bearer ' + token,
        },
      });
      ...
      if (res.status === 401) {
        ...
        if (newToken) {
          res = await fetch('https://votjuphsggdecoawqeqc.supabase.co/rest/v1/users?id=eq.' + user.id + '&select=user_type', {
            headers: {
              apikey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZvdGp1cGhzZ2dkZWNvYXdxZXFjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE4NTE3OTQsImV4cCI6MjA5NzQyNzc5NH0.fZW6kk62ka7glAPbJiNrqrh2UU84v3RNuR_e5w04mKE',
